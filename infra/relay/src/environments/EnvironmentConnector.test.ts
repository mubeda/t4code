...
function signMintResponse(
  ...
  const payload = {
    iss: `t4code-env:${requestProof.environmentId}`,
...
describe("EnvironmentConnector", () => {
  ...
  it.effect("checks linked environment health through the managed endpoint", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(seenUrls).toEqual(["https://env.example.test/api/t4code-connect/health"]);
      expect(seenProofs[0]).toMatchObject({
        iss: "https://relay.example.test",
        aud: "t4code-env:env-connector-test",
        sub: "user_123",
        environmentId: "env-connector-test",
  ...
  it.effect("mints a one-time environment credential through the linked endpoint", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(seenUrls).toEqual(["https://env.example.test/api/t4code-connect/mint-credential"]);
      expect(seenProofs[0]).toMatchObject({
        iss: "https://relay.example.test",
        aud: "t4code-env:env-connector-test",
        sub: "user_123",
