...
describe("relayResourceNameForStage", () => {
  it("isolates production and personal stages", () => {
    expect(relayResourceNameForStage("t4code-relay-traces", "prod")).toBe(
      "t4code-relay-traces-prod",
    );
    expect(relayResourceNameForStage("t4code-relay-traces", "dev_julius")).toBe(
      "t4code-relay-traces-dev-julius",
    );
  });
});
...
describe("managed endpoint names", () => {
  it("uses the stage slug and a stable stage-scoped digest suffix", () => {
    ...
    expect(managedEndpointHostname("prod", "t4coderelay.com", hash)).toBe(
      "prod-abcdef0123456789.t4coderelay.com",
    );
    expect(managedEndpointTunnelName("dev_julius", hash)).toBe(
      "t4coderelay-managedendpoint-dev-julius-abcdef0123456789",
    );
  });
