          ...
          FileSystem.layerNoop({
            exists: (path) =>
              input.legacyPathProbeError
                ...
                : Effect.succeed(
                    input.legacyPathExists === true && path.includes("T4Code (Alpha)"),
                  ),
            readFileString: () =>
...
describe("DesktopAppIdentity", () => {
  it.effect("keeps using the legacy userData path when it already exists", () =>
    withIdentity(
      Effect.gen(function* () {
        ...
        assert.equal(userDataPath, "/Users/alice/Library/Application Support/T4Code (Alpha)");
      }),
  ...
  it.effect("preserves failures while inspecting the legacy userData path", () => {
    const legacyPath = "/Users/alice/Library/Application Support/T4Code (Alpha)";
  ...
  it.effect("configures app identity from the environment commit override", () => {
    ...
    return withIdentity(
      Effect.gen(function* () {
        ...
        assert.deepEqual(calls.setName, ["T4Code (Alpha)"]);
        assert.equal(calls.setAboutPanelOptions[0]?.applicationName, "T4Code (Alpha)");
