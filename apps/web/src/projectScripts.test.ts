...
import {
  ...
  setupProjectScript,
} from "@t4code/shared/projectScripts";
...
describe("projectScripts helpers", () => {
  ...
  it("builds default runtime env for scripts", () => {
    ...
    expect(env).toMatchObject({
      ...
      T4CODE_WORKTREE_PATH: "/repo/worktree-a",
    });
  });
  ...
  it("allows overriding runtime env values", () => {
    const env = projectScriptRuntimeEnv({
      project: { cwd: "/repo" },
      extraEnv: {
        T4CODE_PROJECT_ROOT: "/custom-root",
        CUSTOM_FLAG: "1",
      },
    });

    expect(env.T4CODE_PROJECT_ROOT).toBe("/custom-root");
    expect(env.CUSTOM_FLAG).toBe("1");
    expect(env.T4CODE_WORKTREE_PATH).toBeUndefined();
  });
