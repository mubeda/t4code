import { relayClerkTokenOptions } from "@t4code/shared/relayAuth";
...
export class CloudPublicConfigMissingError extends Schema.TaggedErrorClass<CloudPublicConfigMissingError>()(
  ...
  {
    key: Schema.Literal("T4CODE_CLERK_JWT_TEMPLATE"),
  },
) {
...
export function resolveCloudPublicConfig(): CloudPublicConfig {
  return {
    ...
    relayUrl: normalizeSecureRelayUrl(
      (import.meta.env.VITE_T4CODE_RELAY_URL as string | undefined) ?? "",
    ),
    relayTracing: {
...
export function resolveRelayClerkTokenOptions() {
  ...
  if (!clerkJwtTemplate) {
    throw new CloudPublicConfigMissingError({ key: "T4CODE_CLERK_JWT_TEMPLATE" });
  }
