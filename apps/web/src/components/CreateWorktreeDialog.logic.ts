...
// --- AddProjectDialog folder-of-repos detection ----------------------------
// Shared here (rather than a separate AddProjectDialog.logic.ts) because the
// helper is a handful of lines and AddProjectDialog has no other pure logic
// worth splitting out.

export interface BrowseEntryLike {
  readonly name: string;
  readonly fullPath: string;
}

/** True when a directory's own entries contain a `.git` entry (it is itself a repo). */
export function hasGitEntry(entries: ReadonlyArray<BrowseEntryLike>): boolean {
  return entries.some((entry) => entry.name === ".git");
}

export interface ChildRepoScan {
  readonly name: string;
  readonly fullPath: string;
  readonly hasGit: boolean;
}

export interface FolderOfReposDetection {
  readonly selectedIsRepo: boolean;
  readonly repoCandidates: ChildRepoScan[];
  /** True when the selected directory is not itself a repo, but contains >=1 child repo. */
  readonly isFolderOfRepos: boolean;
}

/**
 * Given the entries of the selected directory and a (bounded, already-fetched)
 * scan of its immediate children for `.git` presence, decides whether to
 * offer the "folder of repos" multi-import UI. Pure — the actual child
 * scanning (one `filesystem.browse` call per child) happens at the call site
 * in `AddProjectDialog`.
 */
export function detectFolderOfRepos(
  selectedEntries: ReadonlyArray<BrowseEntryLike>,
  childScans: ReadonlyArray<ChildRepoScan>,
): FolderOfReposDetection {
  const selectedIsRepo = hasGitEntry(selectedEntries);
  const repoCandidates = childScans.filter((c) => c.hasGit);
  return {
    selectedIsRepo,
    repoCandidates,
    isFolderOfRepos: !selectedIsRepo && repoCandidates.length > 0,
  };
}
