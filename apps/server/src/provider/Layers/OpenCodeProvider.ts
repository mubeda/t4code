        ...
        probe: {
          ...
          message:
            openCodeSettings.serverUrl.trim().length > 0
              ? "OpenCode is disabled in T4Code settings. A server URL is configured."
              : "OpenCode is disabled in T4Code settings.",
        },
      });
    }
...
export const checkOpenCodeProviderStatus = Effect.fn("checkOpenCodeProviderStatus")(function* (
  ...
  if (!isExternalServer) {
    ...
    if (!version) {
      return fallback(
        new Error(
          `Unable to determine OpenCode version from \`opencode --version\` output. T4Code requires OpenCode v${MINIMUM_OPENCODE_VERSION} or newer.`,
        ),
        null,
      );
    }
