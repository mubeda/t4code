...
import { RelayEnvironmentConnectScope, RelayEnvironmentStatusScope } from "@t4code/contracts/relay";
      ...
      return yield* Effect.tryPromise({
        try: () => readCachedClerkToken(nowMillis),
          ...
          new ManagedRelaySessionError({
            message: "Could not obtain the T4 Cloud session token.",
            cause,
          }),
      });
    }),
  ...
  return session.readClerkToken().pipe(
    Effect.flatMap((token) =>
      token
        ...
        : Effect.fail(
            new ManagedRelaySessionError({
              message: "The T4 Cloud session token is unavailable.",
            }),
          ),
    ),
  );
...
function requireClerkToken(
  ...
  if (!session || session.accountId !== accountId) {
    return Effect.fail(
      new ManagedRelaySessionError({
        message: "Sign in to T4 Cloud before loading relay data.",
      }),
    );
  }
...
export function readManagedRelaySnapshotState<A>(
  ...
  if (result._tag === "Failure") {
    ...
    error = cause instanceof Error ? cause.message : "Could not load T4 Cloud data.";
