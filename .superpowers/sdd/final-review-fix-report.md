...
Windows process-tree ownership before a batch shim can create descendants.

One platform-verification limitation remains: this host is macOS, so the three
new `#[cfg(windows)]` real-process regressions could not execute here. The actual
production Windows executable, job-object, and PTY modules pass an isolated
`x86_64-pc-windows-msvc` typecheck. Host-independent Windows construction tests
also pass, and the real shim, gate-ordering, and descendant-kill regressions are
checked in for a Windows runner.
...
Portable PTY does not expose a pre-spawn creation-flags hook, so its batch path
starts the current T4Code executable with an internal marker, unique supervision
gate, ready-event name, script path, and provider arguments as structured argv.
Both the server CLI and Tauri desktop entrypoints recognize the marker. The
trampoline signals that it reached the gate and cannot start the target batch file
until the PTY parent has attached the trampoline root to its Windows job and
signals authorization. This avoids a raw application-owned `cmd /c` command
string, avoids transport environment variables that could clobber or leak caller
state, and prevents descendants from escaping the terminal job during the spawn
window. Script paths containing spaces and `!` remain structured values; script
paths containing control characters are rejected. Unix still launches the
...
after the gate and ready-event protocol was added.
...
unchanged caller-owned `T4CODE_INTERNAL_BATCH_SCRIPT` value. Two more Windows
tests prove that the trampoline reaches but cannot pass its supervision gate
before authorization, and that killing a batch terminal terminates its spawned
PowerShell descendant.
...
```text
RUSTC="$(rustup which --toolchain 1.97.1 rustc)" \
  rustup run 1.97.1 cargo check --offline --target x86_64-pc-windows-msvc
PASS
```

The temporary check crate and artifacts were removed after verification. The
real-process tests still require a Windows host.
...
4. A second review found that the trampoline could start its target batch file
   before the PTY parent attached the trampoline process to a job, allowing a
   descendant to escape kill-on-close ownership. PTY batch launches now wait on a
   named authorization gate until after attachment. Native/process-wrap launches
   use `CREATE_SUSPENDED`, attach the root to a job, and resume its thread only
   after successful attachment. The provider inventory's Git process runner now
   uses the same supervised background policy, closing the remaining native
   provider-probe path.
...
On Windows, a portable-PTY batch trampoline cannot create `cmd.exe` until its
already-spawned root belongs to the terminal job. Every supervised native
background root is suspended before user code runs, attached to a job, and only
then resumed. Attachment/resume failures terminate both the job and root. The
job's `KILL_ON_JOB_CLOSE` policy covers descendants on explicit kill and on later
setup failures.
...
`batch_trampoline_waits_for_the_parent_supervision_gate`, and
`killing_a_batch_terminal_terminates_its_descendant_process`. The macOS host
validated exact Windows argv construction, cross-platform behavior, and compiled
the actual Windows-specific production modules, but cannot prove
`cmd.exe`/PowerShell/ConPTY runtime behavior. No code, test, lint, typecheck, or
wire-parity failure remains on the available host.
