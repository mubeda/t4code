...
This guide is for people who want to use more than one Codex account in T4Code.
...
In T4Code Settings, name it something obvious:
...
In T4Code Settings, add another Codex provider:
...
T4Code shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, T4Code treats it
