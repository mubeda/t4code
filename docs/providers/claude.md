...
This guide is for people who want to use more than one Claude setup in T4Code.
...
In T4Code Settings, your Claude provider can stay like this:
...
An empty `Claude HOME path` means T4Code uses your normal home directory.
...
Then add another Claude provider in T4Code:
...
T4Code only offers Claude providers that use the same Claude home for an existing thread. A
...
multiple files under its home directory, so T4Code keeps separate Claude homes isolated instead of
...
Add or edit a Claude provider in T4Code Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. T4Code stores the value as a server secret and does not
...
T4Code does not need a special Claude Code Router provider. Treat the router as a Claude
environment.
...
2. Add or configure a Claude provider in T4Code.
