...
- **Monorepo tooling is `vite-plus` (`vp`), not npm/vitest directly.** In this environment **pnpm is invoked as `corepack pnpm`** (pnpm is not on PATH; corepack activates the pinned `pnpm@10.24.0`). A benign `WARN Unsupported engine` (node 24.12 vs wanted 24.13) prints on every command — ignore it. Commands (run from repo root `X:\Workspaces\Orca\t4code\source-control`):
  ...
  - Typecheck one package: `corepack pnpm --filter @t3tools/web exec tsgo --noEmit` (web) / `corepack pnpm --filter @t3tools/server exec tsc --noEmit` (server) — or repo-wide `corepack pnpm tc`.
  - Test one file (web): `corepack pnpm --filter @t3tools/web exec vp test run --project unit src/rightPanelStore.test.ts`. Test one file (server): `corepack pnpm --filter @t3tools/server exec vp test run src/vcs/GitVcsDriverCore.test.ts`.
